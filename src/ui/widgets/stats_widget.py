...
class StatsWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Identity Group
        self.group_identity = QGroupBox("Character Identity")
        form_identity = QFormLayout(self.group_identity)
        ...
        self.txt_steam_id.setPlaceholderText("Only for PC saves")
        self.txt_steam_id.textChanged.connect(self.stats_changed.emit)
        self.row_steam_id = form_identity.addRow("SteamID:", self.txt_steam_id)
        self.group_identity.layout().itemAt(1).widget().hide() # Hide SteamID label by default
        self.txt_steam_id.hide()
        
        layout.addWidget(self.group_identity)
    ...
    def load_stats(self, stats_data, steam_id=None):
        ...
        if steam_id is not None:
            self.txt_steam_id.setText(str(steam_id))
            self.txt_steam_id.show()
            # Show label (a bit hacky in QFormLayout)
            self.group_identity.layout().labelForField(self.txt_steam_id).show()
        else:
            self.txt_steam_id.hide()
            label = self.group_identity.layout().labelForField(self.txt_steam_id)
            if label: label.hide()
    ...
    def get_stats(self):
        ...
        return {
            ...
            "steam_id": self.txt_steam_id.text() if not self.txt_steam_id.isHidden() else None
        }
