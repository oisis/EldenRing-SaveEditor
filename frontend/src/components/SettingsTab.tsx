...
function validateSteamId(value: string): string {
    return /^\d+$/.test(value.trim()) ? '' : 'SteamID must contain decimal digits only.';
}
...
export function SettingsTab({
    ...
    const handleSteamIDSubmit = () => {
        ...
        const err = validateSteamId(trimmed);
        if (err) {
            setSteamIDError(err);
            return;
        }
    ...
    const handleApplySteamId = async () => {
        ...
        try {
            await SetSteamIDFromString(trimmed);
            setSteamIdInput(trimmed);
            setSteamIdSaved(trimmed);
        }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    <div className="card p-6 w-full max-w-sm space-y-4">
                        ...
                        <p className="text-[9px] text-muted-foreground">
                            Enter the exact numeric ID generated by Steam or your emulator.
                            An incorrect ID may prevent the converted save from loading.
                        </p>
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            {platform !== 'PC' ? (
                                ...
                                <input type="text" value={steamIdInput} onChange={e => { setSteamIdInput(e.target.value); setSteamIdError(''); }}
                                    placeholder="Steam or emulator ID"
