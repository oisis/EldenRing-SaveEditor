...
### fix(gaitem): preserve native allocation and mutation semantics

New instances now use native free positions and the canonical empty marker.
Structural checks reject non-canonical ordering, while duplicate repair keeps
acquisition buckets and Ash of War partitions consistent. Talisman transfers
remain handle-only, and removing an item now rebuilds the GaItem projection
before another item is added.

### fix(items): add Physick items independently

Crimson Crystal Tear and Flask of Wondrous Physick can now be added separately
or together without creating an automatic Third Church pickup bundle.

### fix(settings): accept emulator Steam IDs

The Steam ID editor and conversion form now accept decimal IDs generated by
Steam or an emulator without requiring the historical `7656119` prefix.
