...
`MoveOwnedItemsToStorage` moves every named Inventory common record into
Storage common as **one atomic mutation of one save revision**. Each record is
...
[`MoveOwnedItemToStorage`](move_owned_item_to_storage.md) endpoint uses.
...
| Implementation source | [../../../backend/endpoints/inventory/move_owned_items_to_storage.go](../../../backend/endpoints/inventory/move_owned_items_to_storage.go) |
...
shared Safety Profile policy — the Storage limit of that item under the active
