...
| `backend/endpoints/character/` | Exists | Character endpoint contracts and implementations. `GetSaveCharacters`, `GetCharacterProfile`, `GetCharacterStats` and `GetCharacterAppearance` are implemented; the remaining character endpoints are contracts only. | The implemented endpoints delegate to SaveEngine and hold no format or session rule of their own. `GetCharacterAppearance` has no transport yet and is reachable from Go callers only. The contract-only endpoints will require SaveEngine and a save session. |
