...
| `backend/endpoints/equipment/` | Exists | Equipment endpoint contracts and implementations. `GetEquipment` is implemented and returns the 22 raw `ChrAsmEquipment` fields of one character slot, and `GetQuickItems` is implemented and returns the ten raw `EquipItemData` Quick Items records of one character slot together with the raw active-slot value; the remaining equipment endpoints are contracts only. | Both implemented getters delegate to SaveEngine and hold no format or session rule of their own. They read no GameCatalog and resolve no `ItemDocument`, they share no helper and `GetQuickItems` calls no other endpoint. The contract-only endpoints will require SaveEngine, and the mutating ones are not implemented. |
