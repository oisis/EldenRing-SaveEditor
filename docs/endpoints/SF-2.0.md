...
| `backend/endpoints/character/` | Exists | Character endpoint contracts and implementations. `GetSaveCharacters` is implemented; the remaining character endpoints are contracts only. | The implemented endpoint delegates to SaveEngine and holds no format or session rule of its own. The contract-only endpoints will require SaveEngine and a save session. |
...
| `backend/saveengine/pc.go` | Exists | PC container recognition by the `BND4` magic, validation of the header, the declared BND4 entry count and the bounds of the ten slots and UserData10, and the PC position of the UserData10 data behind its MD5 prefix. | PC-specific behavior stays PC-specific. The MD5 prefix is skipped, never parsed or verified. No `SteamID`, `UserData11` or slot-block parsing. |
| `backend/saveengine/ps4.go` | Exists | PS4 container recognition by the `CB 01 9C 2C` magic, validation of the fixed header and the bounds of the ten slots and UserData10, and the PS4 position of the UserData10 data, which carries no MD5 prefix. | PS4-specific behavior stays PS4-specific. No `SteamID`, `UserData11`, MD5 or slot-block parsing. |
| `backend/saveengine/characters.go` | Exists | Safe reading of the ten character-slot summaries from the private session snapshot: the UserData10 activity flags and, for an active slot only, the confirmed name and level. | Read-only and non-mutating. Returns no snapshot byte; an inactive slot reports an empty name and level 0 even when residual data remains in the file. |
