...
| `backend/endpoints/character/` | Exists | Character endpoint contracts and implementations. `GetSaveCharacters` and `GetCharacterProfile` are implemented; the remaining character endpoints are contracts only. | The implemented endpoints delegate to SaveEngine and hold no format or session rule of their own. The contract-only endpoints will require SaveEngine and a save session. |
