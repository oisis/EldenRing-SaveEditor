...
| `backend/endpoints/equipment/` | Exists | Equipment endpoint contracts and implementations. `GetEquipment` is implemented and returns the 22 raw `ChrAsmEquipment` fields of one character slot; the remaining equipment endpoints are contracts only. | `GetEquipment` delegates to SaveEngine and holds no format or session rule of its own. It reads no GameCatalog and resolves no `ItemDocument`. The contract-only endpoints will require SaveEngine, and the mutating ones are not implemented. |
