...
| `backend/swagger/` | Exists | Local developer HTTP/OpenAPI explorer over the implemented GameCatalog getters and, in loopback mode only, over the read-only save-session lifecycle, the implemented character getters, including `GetCharacterAppearance`, and the implemented equipment getters `GetEquipment`, `GetQuickItems` and `GetPouchItems`. | Not the application frontend and not part of the application runtime. It creates one shared SaveEngine for its own process. The save-session, character and equipment routes are registered only when the explorer runs without `-allow-external-bind`; with an external bind they do not exist and answer 404. The equipment routes are `GET /api/v1/save-sessions/{saveSessionID}/characters/{characterID}/equipment`, `GET /api/v1/save-sessions/{saveSessionID}/characters/{characterID}/quick-items` and `GET /api/v1/save-sessions/{saveSessionID}/characters/{characterID}/pouch-items`, and all of them return raw state without GameCatalog resolution. It exposes no save write, no setter and no mutation beyond `LoadSave` and `CloseSave`, which create and release an in-memory session without modifying the source file. |
...
| `backend/endpoints/equipment/` | Exists | Equipment endpoint contracts and implementations. `GetEquipment` is implemented and returns the 22 raw `ChrAsmEquipment` fields of one character slot, `GetQuickItems` is implemented and returns the ten raw `EquipItemData` Quick Items records of one character slot together with the raw active-slot value, and `GetPouchItems` is implemented and returns the six raw `EquipItemData` Pouch records of one character slot; the remaining equipment endpoints are contracts only. | All three implemented getters delegate to SaveEngine and hold no format or session rule of their own. They read no GameCatalog and resolve no `ItemDocument`, they share no helper, and neither `GetQuickItems` nor `GetPouchItems` calls any other endpoint. The contract-only endpoints will require SaveEngine, and the mutating ones are not implemented. |
