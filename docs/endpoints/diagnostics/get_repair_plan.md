...
| `level_mismatch`, `soul_memory_below_minimum` | Neither value has a repair contract of its own. SaveEngine writes the stored level and the lifetime runes only as derived consequences of `SetCharacterStats`, whose lifetime-rune floor is counted from the base level of the character's own starting class. Whether this build may rewrite a stored level to match its attributes is an unresolved contract decision: SaveForge 1.5.8 and 1.6.10 both rated the mismatch a *warning* and marked it explicitly not automatically repairable, while 2.0 rates it an error. Until that is settled, a plan does not settle it. A selected attribute repair still moves both values as the side effect described above. |
