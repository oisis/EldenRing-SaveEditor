...
| Transport status | transport-exposed — `GET /api/v1/catalog/info` of the local OpenAPI explorer (`backend/endpoints/swagger`). No Wails binding and no permanent CLI command reach it. |
...
- The only HTTP route is `GET /api/v1/catalog/info` of the local OpenAPI explorer
  in `backend/endpoints/swagger`, a developer tool.
