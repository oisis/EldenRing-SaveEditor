...
| `dataVersion` | `string` | Version identifier of the shipped catalog data: a content fingerprint, not a human-readable release number. It is the SHA-256 of, in order, the JSON encoding of the manifest with its own `dataVersion` cleared, then the JSON encoding of every document in the exact order `catalog.json` lists them, then every file under `assets/` by path and content hash with the paths sorted lexicographically, each element terminated by a `NUL` byte. It depends on nothing but the shipped data tree, so it can be recomputed from the repository at any time. `TestManifestDataVersionMatchesTheShippedTree` in `backend/gamecatalog/loader` recomputes it on every test run and fails when the stored value no longer describes the data. |
