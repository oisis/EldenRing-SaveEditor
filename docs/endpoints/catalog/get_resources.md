...
- Empty means every kind, which today mixes items, colosseums, regions and
  summoning pools in one page.
- `item`, `colosseum`, `region` and `summoning_pool` are the kinds the current
  schema declares, so
...
- The search runs against `Resource.Key` and against the resource name: the item
  ...
  (`Colosseum.Name.Value`), region name (`Region.Name.Value`) or summoning pool
  name (`SummoningPool.Name.Value`).
...
| `resourceType` is neither empty nor `item`, `colosseum`, `region` or `summoning_pool` | `unsupported resource type "…"` |
...
- `item`, `colosseum`, `region` and `summoning_pool` are the resource kinds the
  schema declares today. The `family` and `capability` filters describe items
  only and never match a non-item resource.
