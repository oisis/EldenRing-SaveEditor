...
| `SetOwnedItemQuantity` | `set_owned_item_quantity` | Mutation | `inventory` | implemented (quantity of one owned instance, in the session's private snapshot) | not exposed — no HTTP route, no OpenAPI operation, no Scalar entry, no Wails binding, no CLI command and no frontend; a mutation cannot be exposed before `WriteSave` exists | [inventory/set_owned_item_quantity.md](inventory/set_owned_item_quantity.md) |
