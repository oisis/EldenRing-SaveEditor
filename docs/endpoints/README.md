...
| `SetOwnedItemQuantity` | `set_owned_item_quantity` | Mutation | `inventory` | implemented (quantity of one owned instance, in the session's private snapshot) | not exposed — no HTTP route, OpenAPI operation, Scalar entry, Wails binding, CLI command or frontend; transport exposure is a separate task now that `WriteSave` exists | [inventory/set_owned_item_quantity.md](inventory/set_owned_item_quantity.md) |
