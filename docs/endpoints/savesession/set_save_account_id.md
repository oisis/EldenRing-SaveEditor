...
The result carries the session and its new revision only. The identifier is
private account data: it never appears in a result, an error message, or a log,
so a rejected value is never echoed back to the caller.
