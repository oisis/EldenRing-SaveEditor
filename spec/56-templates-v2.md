...
- **Apply contract** *(historical — this subsection records the Phase 7c-era contract; the **Talisman5** rules below were superseded by Phase 7d.4b / commit `c3c1780e`: Talisman5 now has **no native write contract** — it is never exported and Apply skips it (both equip **and** clear) with `equipment_slot_invalid`. `equipmentSlotKindForKey` is the single source of truth for the writable slot set (talisman1..4 only). Read the Talisman5 sentences here as former behaviour, not current code)*:
  ...
  - **Explicit clear**: `baseItemID: 0` writes `0xFFFFFFFF` (empty slot marker) into the writable slot. *(Phase 7c-era wording said "regardless of slot index — including Talisman5"; superseded — Talisman5 no longer has a write contract and its clear is skipped with `equipment_slot_invalid`.)*
  ...
  - **Talisman5 (slot index 21) policy** *(superseded by Phase 7d.4b / `c3c1780e`)*: Phase 7c warned + skipped a populated Talisman5 (`baseItemID > 0`) but still allowed a `baseItemID = 0` clear to write `0xFFFFFFFF`. **Current code**: Talisman5 has no native write contract, so it is never exported and Apply skips it — both equip and clear — with `equipment_slot_invalid`. Vanilla Elden Ring caps the Pouch at 4 active slots; slot 5 exists in the binary but is unreachable through in-game gameplay.
...
- **Manual validation**: 2026-06-01 *(records the Phase 7c-era contract; the Talisman5 clear result was later superseded by Phase 7d.4b / `c3c1780e` — Talisman5 now has no write contract and Apply skips both equip and clear with `equipment_slot_invalid`)* — equipment export with active talismans + Talisman5 clear; talisman keys appearing in the v2 preview row + Apply enabled; equipment-only Apply happy path with game-load reload + Save & Quit retaining state; pouch gating with `TalismanSlots = 0` (slot 1 OK, slots 2/3/4 `talisman_slot_pouch_insufficient` warn + skip); mixed `profile.talismanSlots = 3 + equipment.talisman4` lifting the cap inside a single apply; `talisman5 baseItemID > 0` warn + skip; `talisman5 baseItemID = 0` clear OK (Phase 7c-era); missing talisman item warn + skip with other slots committing; `equipment + inventory.workspace` hard reject unchanged; regression sweep across Phase 5 / 5D.2 / 6 / 6b / 7a / 7a.2 / 7b.1 / 9 / v1 SortOrderTab paths — user-confirmed `manual OK`.
...
### Phase 7d — spell loadout import + apply (shipped 2026-06-02, sub-phases 7d.0 → 7d.4)

The original single Phase 7d shipped as five disciplined sub-phases on
`feature/templates-v2-spell-writer-foundation`. Each sub-phase carries
its own review gate per the workflow in `~/.claude/CLAUDE.md`. The full
chain (7d.0 core writer foundation → 7d.1 schema/DTO/validation → 7d.2
export builder + import preview/summary → 7d.3 backend apply + bindings
regen → 7d.4 frontend UI) was user-confirmed `manual OK` for the
import / preview / apply flow on 2026-06-02 (Test A — full 14-slot
loadout — and Test B — partial leave-unchanged — both passed; see
§17a.2j). The follow-up Phase 7d.4b (create-from-character export for
equipment + spells + `CreateTemplateV2Modal` checkboxes) shipped
2026-07-29 (see below).

#### Phase 7d.0 — core spell writer foundation (shipped 2026-06-02)

- **Status**: ✅ Shipped 2026-06-02 (commit `6cb2e60`, branch `feature/templates-v2-spell-writer-foundation`).
- **Goal**: introduce a single-slot writer for the 14-slot `EquippedSpells` region with strict pre-validation and no hash side effects, mirroring the discipline of `PatchWeaponItemID` and the eventual Phase 7d.3 batch writer.
- **Files (shipped scope)**: `backend/core/spell_writer.go` (new — `PatchEquippedSpell(slot, slotIndex, spellID) error`, constants `EquippedSpellSlotCount = 14`, `EquippedSpellSlotSize = 8`, `EquippedSpellEmptySentinel = 0xFFFFFFFF`, `EquippedSpellOccupiedFollower = 0xFFFFFFFF`), `backend/core/spell_writer_test.go` (new — 9 PatchEquippedSpell cases covering nil slot, slot-index range, uninitialised offset, out-of-bounds, idempotent no-op, clear via sentinel, occupied write with follower 0xFFFFFFFF, no mutation on validation failure, hash bytes left untouched).
- **Backend impact**: single-slot writer that decodes `spellID == 0xFFFFFFFF → (spell_id=0xFFFFFFFF, follower=0x00000000)` and `spellID != 0xFFFFFFFF → (spell_id=spellID, follower=0xFFFFFFFF)`. No hash mutation here — hash[10] recompute lives in the Phase 7d.3 batch writer.
- **Out of scope**: batch writes, hash recompute, App method, bindings, template wiring.
- **Manual validation**: deferred to Phase 7d.4.

#### Phase 7d.1 — schema, DTO, validation (shipped 2026-06-02)

- **Status**: ✅ Shipped 2026-06-02 (commit `7ac60d0`).
- **Goal**: add a strict typed schema section `sections.spells` carrying 14 named `Spell1..Spell14 *SpellSlotRef` pointers, with the selection model mirroring equipment.
- **Files (shipped scope)**: `backend/templates/schema.go` (+`TemplateSections.Spells *SpellsSection`, +`TemplateSelection.Spells *SectionSelection`, +`SpellsSection` with 14 named fields `Spell1..Spell14`, +`SpellSlotRef { BaseItemID uint32; Name string }`, +constants `SpellSlotCount = 14`, `SpellItemIDPrefix = 0x40000000`, `SpellItemIDPrefixMask = 0xF0000000`, +`SpellSlotOrder []string` canonical iteration order `"spell1"..."spell14"`, +`spellsSelectionFields` allowlist, +validators `validateSpellsSelection` / `validateSpellsSection` / `validateSpellSlotRef`, +private `spellSlotRef(sec, slotKey)` getter; `TemplateSelection.HasAnySelected` extended). `backend/templates/schema_spells_test.go` (new — 17 cases covering JSON/YAML round-trip, `HasAny` semantics, validation happy/rejection paths, prefix check, named-field structure).
- **Apply contract**:
  - **Slot count**: exactly 14 named fields. Out-of-range indexing is structurally impossible.
  - **Prefix**: `SpellSlotRef.BaseItemID` must carry the `0x40000000` mask. Both sorceries AND incantations share this prefix in the SaveForge DB (`backend/db/data/sorceries.go`, `incantations.go`); the earlier briefing that suggested `0x60XXXXXX` for incantations was wrong and is intentionally **not** documented here. The validator rejects any non-`0x4XXXXXXX` value at ingest.
  - **Empty slot**: `BaseItemID == 0` is the explicit-clear sentinel. The save-level sentinel `0xFFFFFFFF` is internal to the writer and intentionally never appears in the public schema.
  - **Omitted slot**: a nil `*SpellSlotRef` pointer means "leave unchanged" — the apply layer never writes that slot. Distinguished from explicit clear (BaseItemID = 0) by pointer nullity.
  - **`Name`**: informational only; the apply layer never reads it. Producers may set it for human readability.
- **Backend impact**: schema-only — no apply path is changed until Phase 7d.3.
- **Out of scope**: builder, preview, apply.

#### Phase 7d.2 — export builder + import preview/summary (shipped 2026-06-02)

- **Status**: ✅ Shipped 2026-06-02 (commit `fad1315`).
- **Goal**: ship the export builder side of the spells section and surface presence in the import preview summary so the frontend can render it without an apply path.
- **Files (shipped scope)**: `backend/templates/export_v2.go` (+`ExportV2Options.EquippedSpellsRaw []uint32` — 14 raw IDs read from `slot.Data` at the EquippedSpells region; strict reject when the length is not `SpellSlotCount` AND `selection.spells` is selected; +`buildSpellsSection(rawIDs, sel)` mapping raw `0xFFFFFFFF` → `&SpellSlotRef{BaseItemID: 0}` (explicit clear) and other raw → `&SpellSlotRef{BaseItemID: SpellItemIDPrefix | rawID}`; `Name` left empty — no DB lookup inside `backend/templates`), `backend/templates/export_v2_spells_test.go` (new — 9 builder cases including strict-length reject, raw-sentinel mapping, per-field vs all selection, prefix re-application). `backend/templates/import.go` (+`ImportPreviewSummary.SpellSlotsPresent []string` with `json:"spellSlotsPresent,omitempty"`; `PreviewBuildTemplateImport` calls helper `spellSlotsPresent(tpl.Sections.Spells)` mirroring `equipmentSlotsPresent`; `selectedSectionsForTemplate` adds `"spells"` when `tpl.Selection.Spells.HasAny()`). `backend/templates/import_spells_test.go` (new — 6 cases). `backend/templates/schema.go` adds private `setSpellSlotRef` helper.
- **Backend impact**: export path can now serialise `sections.spells`; preview can report `selectedSections` + `spellSlotsPresent`. **Apply remains blocked** until Phase 7d.3.
- **Out of scope**: backend apply, bindings regen (deferred to 7d.3), `CreateTemplateV2Modal` integration.

#### Phase 7d.3 — backend apply + bindings + `db.ItemIDToMagicParamID` (shipped 2026-06-02)

- **Status**: ✅ Shipped 2026-06-02 (commit `5c3e538`).
- **Goal**: wire the spells section into the v2 apply path through a batch `WriteSpells` writer with targeted hash[10] recompute, add the DB helper that converts full template IDs back to raw MagicParam IDs, and regen the bindings exactly once.
- **Files (shipped scope)**:
  - `backend/core/spell_writer.go` (+`SpellWrite { SlotIndex int; SpellID uint32 }`, +`(s *SaveSlot) WriteSpells(writes []SpellWrite) error` — pre-validates EVERY write before any byte mutation, rejects duplicate slot indices and out-of-range slot indices, dispatches per write through `PatchEquippedSpell`, recomputes **only** hash[10] inline via `binary.LittleEndian.PutUint32(s.Data[HashOffset+10*4:], equipmentHash(readSpellIDs(s.Data, s.EquippedSpellsOffset)))`, +exported test seam `CalculateDynamicOffsets()` wrapping the unexported `calculateDynamicOffsets` so apply tests in `package main` can materialise calibrated `SaveSlot` fixtures without standing up the full Read pipeline). `backend/core/spell_writer_test.go` extends with 9 WriteSpells cases (calibrated hash assertion using MagicPattern + `calculateDynamicOffsets`, divergent-offset assertion against direct `equipmentHash(readSpellIDs(…))` rather than `ComputeSlotHash`, atomic-rollback on validation failure, duplicate-slot rejection, empty-batch no-op).
  - `backend/db/db.go` (+`ItemIDToMagicParamID(itemID uint32) uint32 { return itemID & 0x0FFFFFFF }` — **28-bit mask**, mirroring the existing `ItemIDToHandlePrefix` convention of 4-bit prefix + 28-bit payload; a 16-bit `0x0000FFFF` mask would truncate high-payload spell IDs and is intentionally rejected). `backend/db/item_id_test.go` (new — 4 cases including the `HighPayloadBitsSurvive28BitMask` regression guard with 4 subtests for payloads `0xFFFF`, `0x10000`, `0x12ABCD`, `0x0FFFFFFF`).
  - `backend/templates/schema.go` (+`SpellSlotRefBySlotKey(sec, slotKey) *SpellSlotRef` public getter wrapping the existing private `spellSlotRef` — needed by the resolver outside `package templates`).
  - `app_templates_v2_spells.go` (new — `resolveSpellWrites(slot, sel, sec) ([]core.SpellWrite, []ImportPreviewIssue, error)`. Semantics: `!sel.Selected(key) || ref == nil` → no write (live slot unchanged); `BaseItemID == 0` → `core.EquippedSpellEmptySentinel` (explicit clear); `BaseItemID != 0` → defensive prefix re-check + DB membership check via `db.GetItemData(id).Category == "sorceries" || "incantations"` + `db.ItemIDToMagicParamID` → raw spell ID. Unknown valid-prefix spell ID → `IssueCodeUnknownItem` warning + skip (mirrors the equipment resolver's not-in-inventory pattern). Go error reserved for infrastructure (nil slot, nil section)).
  - `app_templates_v2_apply.go` (+`SpellSlotsApplied int` on `ApplyTemplateV2Result`; +`hasSpells := tpl.Selection != nil && tpl.Selection.Spells.HasAny()`; empty-selection guard extended to `!hasProfile && !hasStats && !hasInventory && !hasEquipment && !hasSpells`; resolver block placed **after** the equipment resolver and **before** the inventory resolver; `WriteSpells` call placed **after** `vm.MapViewModelToSlot` AND **after** `slot.WriteEquipment` — critical order because the VM flush would otherwise overwrite the EquippedSpells region; `applied` list gains `"spells"` when `len(spellWrites) > 0`).
  - `app_templates_v2_apply_spells_test.go` (new — `spellsApplyFixture(t)` builds calibrated `SaveSlot` using `CalculateDynamicOffsets`; 8 cases covering occupied slot, explicit clear, omitted slot leave-unchanged, per-field selection, all selection, unknown BaseItemID warn+skip, coexists with profile, selected-but-section-missing).
  - `frontend/wailsjs/go/models.ts` (+`spellSlotsApplied: number` on `ApplyTemplateV2Result`, +`spellSlotsPresent?: string[]` on `ImportPreviewSummary` — 4 lines content diff). `frontend/wailsjs/runtime/{package.json, runtime.d.ts, runtime.js}` — mode-bit flip 644 → 755 only from the official `wails generate module`, zero content diff.
- **Backend impact**:
  - **Apply contract**: a v2 template carrying `selection.spells` with at least one selected slot now applies spells through `WriteSpells`. The writer touches **only** hash[10] (the EquippedSpells hash); hash[7] (weapons), hash[8] (armor+talismans), and every other hash entry are left untouched. The global `RecalculateSlotHash` is **NOT wired into production**; per-section writers stay responsible for their own hash entries.
  - **Dispatch order**: profile/stats VM flush → equipment write → spells write → inventory.workspace. The spells write deliberately follows the VM flush because `vm.MapViewModelToSlot` rewrites the EquippedSpells region from the cached VM and would clobber any earlier spell writes.
  - **Atomicity**: the existing `rollbackBoth()` closure covers spells writes — any writer failure rolls back both `slot.Data` (including hash[10]) and the workspace.
  - **Combo guards**: spells coexist freely with profile / stats / equipment / inventory.workspace. **No** combo restriction is introduced for spells (unlike the existing `equipment + inventory.workspace` hard reject — spells do not need a fresh `GaMap`).
- **Frontend impact**: bindings-only in this sub-phase (`spellSlotsApplied` / `spellSlotsPresent` added to the typed Wails models). UI source code remains unchanged here — frontend gating + preview rendering land in Phase 7d.4.
- **Out of scope**: frontend gating, preview rendering, `CreateTemplateV2Modal` checkbox.

#### Phase 7d.4 — frontend UI for spells apply (shipped 2026-06-02)

- **Status**: ✅ Shipped 2026-06-02 (commit `9e8aabe`).
- **Goal**: light up the backend apply / preview / library path in the existing frontend modals so users can preview and apply spell templates.
- **Files (shipped scope)**:
  - `frontend/src/components/templates/ImportTemplatePreviewModal.tsx` — `V2_APPLY_SUPPORTED_SECTIONS` extended from `['profile','stats','inventory.workspace','equipment']` to `['profile','stats','inventory.workspace','equipment','spells']`; `spellSlotsPresent = summary?.spellSlotsPresent ?? []` extracted; included in `showV2Meta`; new row `<div data-testid="import-preview-spell-slots">Spell slots: <span>{joined}</span></div>` rendered right after `import-preview-equipment-slots`; unsupported-section disabled-reason tooltip widened to "…profile, stats, inventory.workspace, equipment, and spells in this phase."
  - `frontend/src/components/templates/TemplateLibraryModal.tsx` — `v2HasApplyableSections` OR-list extended with `selectedSections.includes('spells')`. Library sections row continues to render `selectedSections.join(', ')` so `"spells"` appears verbatim without any label mapping change.
  - `frontend/src/components/templates/__tests__/ImportTemplatePreviewModal.test.tsx` — +5 new cases in a `Phase 7d.4 spells section` describe (Apply enabled for spells-only template, spell-slots row renders, row absent when empty, Apply enabled for combined equipment + spells, Apply disabled with the new tooltip text when an unsupported section accompanies spells). Two pre-existing tests that used the literal `'spells'` as an example of "still unsupported" were rewired to `'inventory.unknown'` with a comment.
  - `frontend/src/components/templates/__tests__/TemplateLibraryModal.test.tsx` — +3 new cases in a `Phase 7d.4 spells entries` describe (Apply enabled for spells-only entry, still disabled when save not loaded, sections row contains "spells").
- **Backend impact**: zero — backend was complete after 7d.3.
- **Frontend impact**: apply gating + preview row + library badge all light up for `selection.spells`. `TemplatesShellModal.tsx` is unchanged because its `needsSession` gate only cares about `inventory.workspace`; spells do not require an active Inventory Edit Session. `CreateTemplateV2Modal.tsx` is unchanged — see Phase 7d.4b below.
- **Tests**: `npx vitest run src/components/templates` → 247 / 247 pass (+5 in `ImportTemplatePreviewModal`, +3 in `TemplateLibraryModal`).
- **Manual validation**: 2026-06-02 — user-confirmed `manual OK` for the main import / preview / apply flow. Test A (full 14-slot loadout: 6 occupied + 8 explicit clear, `selection.spells: true`) and Test B (partial leave-unchanged: per-field selection of spell1/spell2/spell3 only, slots 4–14 retain pre-apply state) both passed. Verified spell IDs: Catch Flame `0x40001770` (incantation), Glintstone Pebble `0x40000FA0` (sorcery), Rock Sling `0x40001266` (sorcery), Heal `0x40001915` (incantation), Rancorcall `0x40001388` (sorcery). The earlier briefing that named `0x40001388` as Glintstone Pebble was wrong — that ID is Rancorcall; Glintstone Pebble is `0x40000FA0`. See §17a.2j for the flow walkthrough.
- **Out of scope**: backend create-from-character export of spells; `CreateTemplateV2Modal` Spells checkbox. Both gated by Phase 7d.4b.

#### Phase 7d.4b — create-from-character export bridge for equipment + spells (✅ Shipped 2026-07-29)

- **Status**: ✅ Shipped. The create-from-character flow now captures **equipment** and the 14 equipped **spells** directly from a loaded save — no hand-authored YAML required.
- **Current contract**:
  - `BuildTemplateV2ExportOptions` selection accepts `equipment` and `spells`; `buildAndValidateTemplateV2FromCharacter` builds both sections from **one deep snapshot** of the source slot (`cloneCharacterSlot` → `ReadEquippedState` + `editor.BuildSnapshot`), so every exported section reflects a single, consistent slot state.
  - **Equipment** exports up to **18 supported slots**: 6 weapon, 4 ammo, 4 armor, and the active `talisman1..4`. Talisman slots the source has **not** unlocked are omitted entirely (never emitted as clears). `talisman5` has no native write contract and is never exported.
  - **Spells** exports all **14** slots.
  - Every empty *supported* slot is emitted as an **explicit clear** (`baseItemID: 0`), so applying the template strips stale gear / spells from the target rather than leaving them behind.
  - Equipment is **mutually exclusive** with every Inventory-mutating section — `inventory.workspace`, `items`, `inventoryLayout`, `storageLayout` — on **all three paths**: create-from-character (rejected in `BuildV2Template` and at the UI), import preview (`PreviewBuildTemplateImport` → `equipment_inventory_combo_unsupported` hard error), and direct apply (`ApplyBuildTemplateV2ToCharacterJSON` re-rejects before session gating and any mutation). The reason is the split between the **native Inventory / GaMap** equipment resolves handles from and the **Sort Order workspace** those sections mutate: items they stage or reorder are not committed to `slot.GaMap` until the user clicks Save changes. Equipment **may** be combined with `profile` / `stats` / `spells`.
  - Preview → Save to Library preserves the **exact** canonical JSON (`SaveImportedBuildTemplateJSONToLibrary` stores the previewed payload; `PreviewBuildTemplateFromLibrary` reloads it verbatim), so a create → preview → library → apply round-trip carries the full loadout intact.
  - Equipment / spells **Apply** does not require an open Sort Order workspace (`ApplyTemplateV2Options.SessionID` unused for these sections). An open Inventory Edit Session for the target is still a conflict for a profile / stats / equipment / spells template — the existing session policy blocks the direct character-section mutation with the "close the Sort Order workspace" gate.
- **Files**: `internal/application/app_templates_v2.go` (equipment/spells source builders off the cloned slot), `app_templates_v2_equipment.go` (equipped-armaments scanner + resolver), `backend/templates/export_v2.go` (section-combination gate), `frontend/src/components/templates/CreateTemplateV2Modal.tsx` (equipment + spells checkboxes with the exclusivity gate).
- **Ammo (arrows / bolts)**: ammo stays a pass-through category (`editor.SupportedCategories`) for the Sort Order workspace — it is never added to the editable inventory. The Equipment resolver nonetheless equips **occupied** ammo: for arrows/bolts slots it resolves the ref directly against the target's `Inventory.CommonItems` + `GaMap` (`collectAmmoCandidates`, positive quantity + existing GaMap entry, Storage never scanned) rather than the editable-inventory snapshot, so an owned arrow/bolt equips the same as a weapon or armor. Ammo not present in the target's Inventory is skipped with `equipment_item_not_in_inventory`; ammo **clears** apply as before.
...
### Phase 8D.1 — v2 items apply backend, `addMissing` only (shipped 2026-06-08)

- **Goal**: first real v2 apply path for `sections.items`. Promote
  the section from export-only (Phase 8C.1) to apply-eligible with a
  single mode allowlist — `addMissing` — that never mutates existing
  live items. UI surface lands in Phase 8D.2.
- **Scope**: backend writer + result/summary plumbing. **No UI, no
  schema change, no new sections, no public JSON re-introduction.**
- **Writer wiring (`app_templates_v2_apply.go`)**:
  - New `applyTemplateItemsAddMissingToWorkspace(sess.Workspace,
    sec, weaponOverride)`. Walks `sections.items.entries`, builds
    the match tuple
    `(itemID, category, upgrade, infusion, AoW, container)` per
    entry, and dispatches missing items through the existing
    `editor.AddItem` / workspace add helper so storage and
    inventory both route through the same allocator. Duplicate
    same-itemID entries are supported via the template-local
    `entryID` — the schema (Phase 8B) already guarantees `entryID`
    uniqueness, and the resolver de-duplicates on the live side by
    the match tuple.
  - Template entries whose tuple already resolves to a live item
    are **skipped** with `items_already_present` (warning, not
    fatal — the user opted into addMissing).
- **Categories outside `editor.SupportedCategories`** are skipped
  with `unsupported_category` (warning, not fatal — the schema
  allowlist covers all 18 categories but the apply MVP only writes
  the ones the editor's add pipeline already supports). The notice
  carries the entryID and the category string so the user can see
  exactly which row was dropped.
- **`quantity == 0`** is rejected at the schema validator (Phase
  8B); the apply path never sees them. `clear` / `remove` semantics
  belong to a future mode, not the entry payload.
- **Weapon level override** (Phase 7a.2) is honoured for newly
  added weapon-like items: after each successful add, the helper
  runs `applyWeaponLevelOverride` so the override applies to
  template additions just like it does to existing weapon patches.
  Templates carry no override field on the wire — the runtime
  option travels through
  `ApplyTemplateV2Options.WeaponLevelOverride` exactly as in
  Phase 6b / 7a.2. Items templates baked with their own
  `applyOptions.weaponLevelOverride` will see that field surfaced
  as `items_template_override_ignored` (info) — runtime override
  always wins.
- **Layout sections** appearing alongside `items` are dropped
  with `items_layout_ignored` (info, not fatal) until Phase 8E.1
  ships the dedicated layout writer. This warning's severity is
  intentionally informational — the apply still succeeds.
- **Session gate**: items section requires an active Inventory
  Edit Session. Same reason as `inventory.workspace`: both mutate
  `sess.Workspace`. No session ⇒ `inventory_session_required`
  (error) and zero side effects. `equipment + items` is allowed;
  `equipment + inventory.workspace` remains hard-rejected
  (Phase 7b.1) — combo guard is preserved.
- **New issue codes** (`backend/templates/import.go`):
...
- **Result fields**: `ApplyTemplateV2Result.InventoryItemsApplied`
  and `…StorageItemsApplied` count rows actually added by this
  apply (not the entry count from the template). Wails bindings
  regenerated.
- **Tests** (`app_templates_v2_apply_items_v2_test.go`,
  ~25 cases): add to empty inventory + storage; duplicate same
  itemID via entryID; categories outside `SupportedCategories`
  skipped + warned; weapon override applied on add; explicit
  `addMissing` mode on the wire; no-session reject; items + layout
  coexistence with the ignored-layout warning; rollback on writer
  failure; preserved existing items; match tuple includes upgrade
  / infusion / AoW.
- **Validation**:
  - `go vet ./backend/templates/... ./` — clean.
...
- **Not implemented in this phase**:
  ...
  - Items modes beyond `addMissing` (`updateExisting`, `merge`,
    `replace`) — out of scope for the v2 items MVP.
  - Layout writers (Phase 8E.1).
  - Manual validation — deferred to Phase 8G.

### Phase 8D.2 — v2 items apply UI controls (shipped 2026-06-08)

- **Goal**: lift the Phase 8D.1 backend behavior into the UI
  without changing any apply semantics. Items-bearing v2 templates
  must be addressable from every existing v2 surface (imported
  preview, library, URL preview, Apply with overrides) on terms the
  user understands.
- **Scope**: frontend-only. **No backend change, no schema
  change, no new bindings, no public JSON re-introduction.**
- **Apply gating**
  (`frontend/src/components/templates/ImportTemplatePreviewModal.tsx`,
  `TemplateLibraryModal.tsx`):
  - `V2_APPLY_SUPPORTED_SECTIONS` adds `'items'`. Items-bearing
    templates now reach the Apply button on the imported preview,
    library list, and URL preview (URL preview reuses the import
    preview modal, so the change is inherited for free).
  - Disabled-Apply tooltip splits — items templates show "Items:
    apply supported (add missing only)"; layout-only templates
    keep the "export-only" reason (until Phase 8E.2 lifts that
    too); items + layout combo shows "Inventory layout / storage
    layout are export-only and will be ignored on apply." to match
    the backend `items_layout_ignored` warning.
- **Session gate** (`TemplatesShellModal.tsx`):
  - The Inventory Edit Session requirement now also covers items
    applies (items mutate `sess.Workspace`, same gate as
    `inventory.workspace`).
- **Result modal** (`TemplatesShellModal.tsx`):
  - New `ApplyItemsResultModal` (`items-apply-result-modal` testid)
    opens after every items apply with `inventoryItemsApplied` /
    `storageItemsApplied` counters from the new
    `ApplyTemplateV2Result` fields. Profile/stats-only applies
    keep the existing toast UX with no extra modal.
  - Modal opens for both imported-preview and library routes so
    the user sees the same per-apply detail regardless of surface.
- **Weapon level override exposure**
  ...
  - Direct Apply (without overrides) keeps using template /
    default upgrade levels; the new `import-preview-items-weapon-hint`
    line (added in Phase 8D.3) makes that contract explicit.
- **Tests**: extensive additions across the four templates test
  files cover the new gating, items confirm row, result modal, and
  weapon override surface.
- **Validation**:
  - `cd frontend && npx tsc --noEmit` — clean.
...
### Phase 8D.3 — items apply polish (shipped 2026-06-08)

- **Goal**: tighten the Phase 8D.2 surface in three places — make
  the explicit `addMissing` mode visible on the wire, group result
  warnings by code, and surface the weapon-level override hint
  uniformly. No schema, no writer, no new sections.
...
- **Explicit `addMissing` on the wire**
  ...
  - New `injectExplicitAddMissing(canonicalJSON)` helper rewrites
    ...
    payload itself.
  - Applied on both the imported preview JSON apply path and the
  ...
  - **Library items entries** now route through
    ...
    `addMissing` rides the wire for the library surface too.
    Non-items entries (profile / stats / equipment / spells) stay
    on the fast FromLibrary path — no extra round-trip noise.
- **Warning grouping in the result modal**
  (`ApplyItemsResultModal`):
  - Warnings are now grouped by code rather than rendered as a
    flat list. Named groups:
    `items-apply-result-already-present`,
    `items-apply-result-unsupported-category`,
    `items-apply-result-layout-ignored`,
    `items-apply-result-weapon-warnings` (covers
    `weapon_level_clamped` and `weapon_unupgradeable`),
    `items-apply-result-template-override-ignored`. Unknown codes
    fall into `items-apply-result-other-warnings` with the code
    prefix shown.
  - Each group caps the visible items at
    `WARNING_PREVIEW_LIMIT = 5` with a `+ N more (total: X)` line
    so a noisy apply doesn't blow up the modal.
  - All groups expose `data-warning-severity="info"` — every code
    surfaced here is informational, not a blocking failure.
- **UI copy**:
  - `ImportTemplatePreviewModal.tsx` renders an
    `import-preview-items-weapon-hint` line ("Direct Apply uses
    template / default upgrade levels. Use 'Apply with overrides…'
    to override standard (+0–25) or somber (+0–10) weapon levels
    for newly added items.") whenever the items section is
    selected.
  - `TemplateLibraryModal.tsx` adds the same hint to the v2
    confirm row (`library-apply-v2-weapon-hint`).
- **Tests**: cover wire-format injection on the imported preview /
  overrides / library routes, warning-group expansion,
  layout-ignored severity stays `info`, weapon hint visibility on
...
### Phase 8E.1 — v2 inventory/storage layout apply backend, `reorderOnly` only (shipped 2026-06-08)

- **Goal**: first real v2 apply path for `sections.inventoryLayout`
  and `sections.storageLayout`. Promote both layout sections from
  export-only (Phase 8C.1) to apply-eligible with a single mode
  allowlist — `reorderOnly` — that never adds, removes, or
  replaces items. UI surface lands in Phase 8E.2.
- **Scope**: backend writer + result/summary plumbing. **No UI, no
  schema change, no new sections, no public JSON re-introduction,
  no new writers outside the workspace reorder helper.**
- **Writer wiring (`app_templates_v2_apply.go`)**:
  - New `applyTemplateLayoutToWorkspace(sess.Workspace,
    container, section, mode)` reorders the workspace list in
    place. The implementation maps each layout entry to a live
    workspace item by entryID → match tuple
    `(itemID, category, upgrade, infusion, AoW, container)` — the
    same tuple the items resolver builds, deliberately keyed by
    the template-local entryID for parity.
  - **Mode allowlist** is `reorderOnly` only:
    - nil / empty / `reorderOnly` → reorder path.
    - `ignore` → documented no-op (the section is silently
      skipped; this is the legal "I have layout in the template
      but don't apply it" escape hatch).
    - `append` / `replace` / any other value → emit
      `layout_mode_unsupported` (warn) and skip the section. The
      UI never sends these; the warning's only purpose is to
      flag hand-authored YAML with an unsupported mode.
- **Match path**:
  - Layout entries whose `entryRef` cannot resolve to a live
    workspace item emit `layout_entry_missing` (warn) and the
    entry is skipped. The apply does **not** add the missing
    item — that is the job of `sections.items` addMissing
    (Phase 8D.1).
  - When two live items match the same template entry, the first
    live match wins so the apply remains deterministic. The
    writer emits `layout_entry_ambiguous` (warn) so the user
    sees the ambiguity.
- **Position normalisation**:
  - Sparse / non-zero-based template positions are accepted and
    normalised to compact `0..N-1`. When the writer actually has
    to normalise (i.e. the template's positions are not already
    dense and zero-based), it emits `layout_sparse_normalized`
    (info) so producers see what changed.
- **Extras preservation**:
  - Live items not referenced by the template layout are NOT
    deleted. They are appended after the template-ordered prefix
    in their original relative order, and the writer emits
    `layout_extra_items_preserved` (info, with the count). This
    is the load-bearing invariant the UI copy in Phase 8E.2
    refers to ("Extras are preserved and appended").
- **Apply ordering**:
  - Items addMissing runs **before** the layout writer. Newly
    added items become eligible for ordering inside the same
    apply — the user does not need a second apply to put fresh
    items in the right place. The "items first, then layout" copy
    in Phase 8E.2 reflects this invariant.
- **Atomicity**:
  - The layout writer dispatches inside the existing Phase 7a
    dual snapshot scope, so any writer failure rolls both
    `slot.Data` (`core.RestoreSlot`) and the workspace value
    snapshot.
- **Session gate**:
  - Layout sections require an active Inventory Edit Session — the
    writer mutates `sess.Workspace`. Without a session the apply
    returns `inventory_session_required` and zero side effects,
    same as items and `inventory.workspace`. A layout-only
    template (no items, no `inventory.workspace`) still requires
    a session because the writer still has to touch
    `sess.Workspace`.
- **New issue codes** (`backend/templates/import.go`):
...
- **Result fields** (`ApplyTemplateV2Result`):
...
- **Tests** (`app_templates_v2_apply_layout_test.go`, 24 cases):
  mode allowlist (`reorderOnly`, `ignore`, `append`/`replace`/
  other), missing / ambiguous / sparse / extras paths, items +
  layout interop ordering (items added first then ordered),
  no-session reject, both containers, atomicity (writer failure
  leaves slot and workspace untouched), warning counts,
  deterministic first-match-wins, weapon match key reuse.
- **Validation**:
  - `go vet ./backend/templates/... ./` — clean.
  ...
  - `go build ./backend/... ./` — green.
- **Not implemented in this phase**:
  - Frontend exposure of the layout apply (Phase 8E.2).
  - `append` / `replace` layout modes — schema-allowed,
    writer-rejected, UI never sends them.
  - Items modes beyond `addMissing`.
  - Manual validation — deferred to Phase 8G.

### Phase 8E.2 — v2 layout apply UI controls (shipped 2026-06-08)

- **Goal**: lift the Phase 8E.1 backend behavior into the UI,
  completing the items + layout milestone. Layout-only templates
  must be addressable; items + layout templates must clearly
  communicate the items-first-then-reorder ordering.
- **Scope**: frontend-only. **No backend change, no schema
  change, no new bindings, no public JSON re-introduction.**
- **Apply gating**
  (`frontend/src/components/templates/ImportTemplatePreviewModal.tsx`,
  `TemplateLibraryModal.tsx`):
  - `V2_APPLY_SUPPORTED_SECTIONS` now includes `inventoryLayout`
    and `storageLayout`. The Phase 8C.1
    `LAYOUT_ONLY_SECTIONS` short-circuit and the "no apply path
    in Phase 8D.2" tooltip are gone.
  - Library `v2HasApplyableSections` widens to enable Apply on
    layout-bearing entries (items-less layout-only templates
    included).
  - The old `library-apply-v2-layout-ignored` confirm-row testid
    is removed; layout-bearing v2 library entries now show
    ...
    - Items + layout: same "Reorder only" header + "Missing
      items are added first, then layout is applied."
  - The preview modal renders
    `import-preview-layout-reorder-only` for layout-only
    templates (four lines of plain-English semantics covering
    reorder-only, "does not add/remove/replace", extras
    preserved, missing skipped).
- **Session gate** (`TemplatesShellModal.tsx`):
  - The Inventory Edit Session requirement now also covers
    layout applies (the writer reorders `sess.Workspace`).
    Layout-only templates require an active session; missing
    session surfaces the same `NO_SESSION_MESSAGE` toast as items
    and `inventory.workspace`. UI cannot reach the apply binding
    without a session.
- **Explicit defaults on the wire** (`TemplatesShellModal.tsx`):
  - `injectExplicitAddMissing` is renamed to
    `injectExplicitApplyDefaults` and extended. The helper writes
    `applyOptions.items.mode = "addMissing"`,
    `applyOptions.inventoryLayout.mode = "reorderOnly"`, and
    `applyOptions.storageLayout.mode = "reorderOnly"` whenever
    the matching section is selected. Backend defaults nil
    layout mode to `reorderOnly` (Phase 8E.1) — the UI just
    makes the intent testable from the JSON payload.
  - Library routing: layout-bearing entries now go through
    `PreviewBuildTemplateFromLibrary` → JSON apply (so explicit
    modes ride the wire), mirroring the items routing from
    Phase 8D.3. Profile/stats/equipment/spells-only entries stay
    on the fast `ApplyBuildTemplateV2FromLibraryToCharacter`
    path.
- **Result modal** (`ApplyItemsResultModal`):
  - Modal aria-label and headline change to "Template apply
    result" (it now covers items, layout, and the items+layout
    combo).
  - New `items-apply-result-layout-counters` section with six
    counters:
    `items-apply-result-layout-{inv,sto}-{applied,missing,extras}`,
    plus a plain-English footnote explaining the reorder-only
    semantics. The section renders only when at least one
    counter is non-zero or a layout warning is present.
  - Five new warning groups for layout codes:
    ...
    All five carry `data-warning-severity="info"` because layout
    warnings are reporting, not blocking.
  - Modal now opens for layout-only applies, not just items.
- **Contract test**: `src/wails-bindings.contract.test.ts` gains
  a contract test pinning the six `ApplyTemplateV2Result.layout*`
  field names so a binding rename would zero the counters loudly
  instead of silently.
- **Tests**:
  - `ImportTemplatePreviewModal.test.tsx`: two updated cases
    (items + layout copy, layout-only enabled with reorder-only
    copy) + tooltip text update.
  - `TemplateLibraryModal.test.tsx`: two updated cases (items +
    layout confirm row uses new copy; layout-only entry Apply
    enabled with reorder-only confirm row).
  - `TemplatesShellModal.test.tsx`: five new Phase 8E.2 cases
    (layout-only without session, layout-only with session +
    reorderOnly on the wire + result modal counters, items +
    layout both modes on the wire, library layout entry routes
    via JSON, result modal warning grouping severity).
- **Validation**:
  - `cd frontend && npx tsc --noEmit` — clean.
  - `cd frontend && npx vitest run src/components/templates
...
- **Not implemented in this phase**:
  ...
  - Items modes beyond `addMissing` (`updateExisting`, `merge`,
    `replace`).
  - Manual validation — deferred to Phase 8G.
  - Public JSON re-introduction — removed in Phase 8A, still
    gone.

### Phase 8F — cumulative docs/spec sweep (shipped 2026-06-08)

- **Goal**: lock the public contract of v2 items + layout
  in spec/56 (EN + PL twins), update CHANGELOG + ROADMAP, and run
  the full automated validation matrix so Phase 8G can be a pure
  manual sweep.
- **Scope**: documentation + automated validation only. **No
  runtime code changes.**
- See §17a.2k below for the cumulative summary of what the items +
  layout apply contract guarantees as of this phase.
- **Validation**:
...
> **Historical (Phase 7c-era flow).** The Talisman5 handling in step 7 below was superseded by Phase 7d.4b / commit `c3c1780e`: Talisman5 has no native write contract, is never exported, and Apply skips it — both equip and clear — with `equipment_slot_invalid`. The steps are kept as a record of the Phase 7c flow, not current code.
...
7. `resolveEquipmentWrites(slot, sel, sec, activeTalismanSlots)` walks `EquipmentSlotOrder`. For each populated talisman slot, the new branch determines the slot ordinal (1..5 via `talismanSlotOrdinal`); a non-empty ref where `ord > MaxActiveTalismanSlots || ord > activeTalismanSlots` emits `talisman_slot_pouch_insufficient` warning + skip. Explicit-clear refs (`baseItemID = 0`) bypass the pouch gate entirely. *(Phase 7c-era: Talisman5 tripped the warning only when populated and a `baseItemID = 0` clear still wrote `0xFFFFFFFF`. Superseded — Talisman5 now has no write contract and both equip and clear are skipped with `equipment_slot_invalid`; the current writable set is talisman1..4.)*
...
### 17a.2j. Phase 7d v2 spells apply flow exercised

1. Hand-author (or export from a future Phase 7d.4b create-from-character path) a v2 YAML carrying `selection.spells` and `sections.spells`. Two manually-validated shapes: **Wariant A** = `selection.spells: true` + 14 explicit `sections.spells.spell1..spell14` entries (mix of `baseItemID: 0x40XXXXXX` and `baseItemID: 0`); **Wariant B** = `selection.spells: {spell1: true, spell2: true, spell3: true}` + only the three nominated slots in `sections.spells` (slots 4–14 omitted → leave-unchanged).
2. From any apply surface — Library entry, `Import YAML from File…`, `Import from URL…`, or `Apply with overrides…` — the preview now reaches the apply layer. `V2_APPLY_SUPPORTED_SECTIONS` in `ImportTemplatePreviewModal.tsx` includes `'spells'`, and `v2HasApplyableSections` in `TemplateLibraryModal.tsx` ORs in `selectedSections.includes('spells')`, so spells-only entries enable Apply without an active Inventory Edit Session (the shell's `needsSession` gate is unchanged — spells do not need a session).
3. `PreviewBuildTemplateImport` populates `Summary.SpellSlotsPresent` via `spellSlotsPresent(tpl.Sections.Spells)` for every non-nil `Spell{N}` pointer (including explicit-clear `BaseItemID == 0`, which is a present slot in the template even though it clears the save slot). `selectedSectionsForTemplate` adds `"spells"` when `tpl.Selection.Spells.HasAny()`. The preview modal renders the result as a new `<div data-testid="import-preview-spell-slots">Spell slots: …</div>` row directly under the `import-preview-equipment-slots` row.
4. The user clicks Apply (or `Apply with overrides…` for profile/stats overrides; the override modal does not surface spells edits). `ApplyBuildTemplateV2ToCharacterJSON` parses the JSON and detects `hasSpells := tpl.Selection != nil && tpl.Selection.Spells.HasAny()`. The empty-selection guard is widened to `!hasProfile && !hasStats && !hasInventory && !hasEquipment && !hasSpells`, so a spells-only apply proceeds.
5. `resolveSpellWrites(slot, tpl.Selection.Spells, tpl.Sections.Spells)` walks `templates.SpellSlotOrder` (`"spell1"..."spell14"`). For each slot:
   - `!sel.Selected(slotKey)` → no write (live save slot unchanged, even when `sections.spells.spell{N}` is populated).
   - `ref == nil` → no write (the section omitted this slot entirely).
   - `ref.BaseItemID == 0` → `core.SpellWrite{SlotIndex: i, SpellID: core.EquippedSpellEmptySentinel}` (explicit clear).
   - `ref.BaseItemID != 0` → defensive prefix re-check against `templates.SpellItemIDPrefix` (`0x40000000`) + `templates.SpellItemIDPrefixMask` (`0xF0000000`) → DB membership check via `db.GetItemData(id).Category` (must be `"sorceries"` or `"incantations"`) → `db.ItemIDToMagicParamID(id) = id & 0x0FFFFFFF` (28-bit mask, NOT 16-bit) → `core.SpellWrite{SlotIndex: i, SpellID: rawID}`. Either of the two safety nets failing emits an `IssueCodeUnknownItem` warning + skip (the equipment-style not-in-inventory pattern); the apply proceeds with the surviving slots.
6. `SaveSlot.WriteSpells(spellWrites)` runs **after** `vm.MapViewModelToSlot` AND **after** `slot.WriteEquipment`. The post-VM placement is critical: the VM flush rewrites the EquippedSpells region from the cached `slot.Player.Spells`, so any spells write that happened before `vm.MapViewModelToSlot` would be silently clobbered. The post-equipment placement keeps hash[7] / hash[8] recomputes (equipment) and hash[10] recompute (spells) cleanly separated.
7. `WriteSpells` pre-validates EVERY write before any byte mutation — duplicate slot indices, out-of-range slot indices, uninitialised `EquippedSpellsOffset`, and out-of-bounds region all abort with zero bytes touched, mirroring `WriteEquipment`'s no-partial-write invariant. Per-write semantic validation (offset bounds, nil slot) is delegated to `PatchEquippedSpell`, which itself never mutates on failure.
8. Each accepted write goes through `PatchEquippedSpell(slot, slotIndex, spellID)`: `spellID == 0xFFFFFFFF → (spell_id=0xFFFFFFFF, follower=0x00000000)`, `spellID != 0xFFFFFFFF → (spell_id=spellID, follower=0xFFFFFFFF)`. Idempotent writes (target bytes already match) are skipped without touching anything.
9. After all per-slot writes commit, `WriteSpells` recomputes **only** hash[10] by re-reading the now-mutated EquippedSpells region: `binary.LittleEndian.PutUint32(s.Data[HashOffset+10*4:], equipmentHash(readSpellIDs(s.Data, s.EquippedSpellsOffset)))`. The recompute uses the same `readSpellIDs` helper that `ComputeSlotHash` uses, so the in-save hash stays bit-equivalent to a full `ComputeSlotHash` for entry [10] without touching any other entry. Hash[7] (weapons), hash[8] (armor + talismans), and every other entry are explicitly **not** rewritten — the global `RecalculateSlotHash` is intentionally **not** wired into production; per-section writers stay responsible for their own hash entries.
10. `result.SpellSlotsApplied` counts the number of writes actually dispatched. `result.applied` gains `"spells"` whenever `len(spellWrites) > 0`. Per-slot warnings flow into `result.preview.warnings` through the standard `ImportPreviewIssue` shape.
11. The user does NOT click `Save changes` for spells — `WriteSpells` commits directly to `slot.Data` (the hash[10] recompute makes the slot self-consistent), mirroring the Phase 7b.1 / 7c equipment write path. `SaveInventoryWorkspaceChanges` continues to be the only commit point for the `inventory.workspace` section. Spells coexist freely with profile / stats / equipment / inventory.workspace; no combo restrictions apply here (unlike the `equipment + inventory.workspace` hard reject — spells do not need a fresh `GaMap`).
12. The atomic-rollback path is the existing `rollbackBoth()` closure — any writer failure (including `WriteSpells`) restores both `slot.Data` (including hash[10]) and the workspace snapshot, so a spells-resolution warning that escalates into an infrastructure error rolls back every prior section in the same apply.
13. Manual validation 2026-06-02 (user-confirmed `manual OK`): Wariant A and Wariant B both ran through the full Import preview → Apply → Save cycle. Verified spell IDs: Catch Flame `0x40001770` (incantations), Glintstone Pebble `0x40000FA0` (sorceries), Rock Sling `0x40001266` (sorceries), Heal `0x40001915` (incantations), Rancorcall `0x40001388` (sorceries). The earlier briefing that named `0x40001388` as Glintstone Pebble was wrong — `0x40001388` is Rancorcall; Glintstone Pebble is `0x40000FA0`. Sorceries AND incantations both use the `0x40000000` prefix in the SaveForge DB (`backend/db/data/{sorceries,incantations}.go`); the earlier assumption of a separate `0x60XXXXXX` incantation prefix is incorrect and is intentionally **not** documented anywhere in this spec.

### 17a.2k. Cumulative summary after Phase 8E.2 — items + layout apply contract

This subsection consolidates the public contract of `sections.items`,
`sections.inventoryLayout`, and `sections.storageLayout` as it stands
at the end of the Phase 8 family (8A → 8E.2). It is meant to be the
load-bearing reference for Phase 8G manual validation and for anyone
extending the items / layout surface later.

**A. Public exchange surface** (since Phase 8A, unchanged)

- YAML is the only publicly exchangeable template format. JSON
  template files cannot be exported, imported, or applied through
  any public Wails App method or UI surface.
- JSON survives strictly as an implementation detail:
  - on-disk library format,
  - internal hand-off carrier between YAML preview, library save,
    overrides apply, and v2 apply paths.
- `App.SaveImportedBuildTemplateJSONToLibrary`,
  `App.ApplyBuildTemplateV2ToCharacterJSON`, and
  `App.ApplyBuildTemplateFromLibrary` remain as internal contracts
  with no user-visible JSON file dialogs.

**B. Items section** (`sections.items`)

- Schema (Phase 8B): flat list of `TemplateItemEntryV2` records
  keyed by template-local `entryID` (stable string slug, unique
  within the template). Each entry carries `itemID`, `category`,
  `quantity ≥ 1`, `location` (`inventory`/`storage`/`both`),
  optional `upgradeKind` (`standard`/`somber`/`none` with the
  matching `upgradeLevel` range), optional `infusionName`,
  optional `ashOfWarItemID` (non-zero when present; omit to mean
  any-AoW).
- Allowlist (Phase 8B): 18 categories mirroring
  `backend/db/data`. The schema accepts every category. The
  **apply MVP** (Phase 8D.1) writes only the categories supported
  by `editor.SupportedCategories`; out-of-allowlist categories
  surface as `unsupported_category` warnings and are skipped —
  never panicked, never fatal.
- Identity model:
  - `entryID` is the **portable reference** (layout entries
    point at it).
  - The **match key for live apply** is the tuple
    `(itemID, category, upgrade, infusion, AoW, container)` —
    the same tuple the items resolver and the layout resolver
    both build.
  - Duplicate same-itemID entries are first-class: two `inv_0007`
    Longswords (one Heavy +25, one Cold +20 in storage) share
    `itemID` but differ on `entryID` and on the upgrade /
    infusion components of the match key. The Phase 8B duplicate
    test is the regression guard.

**C. Inventory / storage layout sections**
(`sections.inventoryLayout`, `sections.storageLayout`)

- Schema (Phase 8B): ordered list of `LayoutEntry{entryRef,
  position}` rows. `entryRef` must resolve to an existing
  `items.entries[*].entryID`. `entryRef` and `position` are each
  unique within a layout. Positions are NOT required to be
  contiguous, dense, or zero-based — the writer normalises.
- Builder (Phase 8C): exports compact 0..N-1 positions after
  per-row skips so freshly built layouts are already canonical.
- Apply (Phase 8E.1): mode allowlist is `reorderOnly` only.
  - nil / empty / `reorderOnly` → reorder.
  - `ignore` → documented no-op.
  - `append` / `replace` / anything else → emit
    `layout_mode_unsupported`, skip the section. UI never sends
    these — the warning exists for hand-authored YAML.
- Invariants (Phase 8E.1):
  - **No add / no remove / no replace.** The layout writer never
    creates or destroys items.
  - **Extras preserved** — live items not referenced by the
    template layout are appended after the template-ordered
    prefix in their original relative order; the writer emits
    `layout_extra_items_preserved` (info, with count).
  - **Missing entries skipped** — layout entries whose
    `entryRef` resolves to no live item emit
    `layout_entry_missing` (warn) and are skipped. The apply
    does not add the missing item — that is the job of
    `sections.items` addMissing.
  - **Ambiguous duplicates** — when two live items match the
    same template entry, the writer emits
    `layout_entry_ambiguous` (warn) and the first live match
    wins (deterministic).
  - **Sparse user-authored positions** — accepted and normalised
    to compact `0..N-1`; the writer emits
    `layout_sparse_normalized` (info) when normalisation
    actually happens.

**D. Apply behavior**

- Modes shipped:
  - `applyOptions.items.mode = "addMissing"` — only supported
    mode for items. UI sends it explicitly via
    `injectExplicitApplyDefaults`; backend defaults nil to
    `addMissing`.
  - `applyOptions.{inventory,storage}Layout.mode = "reorderOnly"` —
    only supported mode for layout. UI sends it explicitly;
    backend defaults nil to `reorderOnly`.
  - `ignore` — legal no-op for either layout section.
- Modes intentionally NOT shipped:
  `append`, `replace`, `updateExisting`, `merge`, and any other
  value beyond the above. Schema accepts them (Phase 8B); the
  apply writer rejects with `layout_mode_unsupported` or the
  items equivalent.
- **Active Inventory Edit Session is mandatory** for items and
  layout (both reorder `sess.Workspace`). No session ⇒
  `inventory_session_required` and zero side effects. Layout-only
  templates still require a session.
- **Items first, then layout** — when both sections are present in
  the same apply, the writer runs items addMissing **before**
  layout reorder so newly added items become eligible for
  ordering inside the same apply.
- **Persistence model is unchanged** — the apply lands in the
  workspace; the user still has to use Save changes /
  `SaveInventoryWorkspaceChanges` to persist the changes to disk.
  The apply path itself never writes `slot.Data` directly.
- **Weapon level override** (`Apply with overrides…`):
  - Available for items-bearing templates through the existing
    `WeaponLevelOverridePanel` (Phase 7a.2).
  - Applies to **newly added weapon-like items only** — after
    each successful add, the helper runs
    `applyWeaponLevelOverride` and clamps via
    `editor.ClampUpgrade`.
  - Direct Apply (no overrides) uses template-baked or default
    upgrade levels; templates baked with their own
    `applyOptions.weaponLevelOverride` see that field surfaced as
    `items_template_override_ignored` — runtime override always
    wins.
- Both **inventory** and **storage** containers are supported for
  layout reorderOnly. The writer routes by container; the apply
  path does not require both layouts to be present.

**E. Warnings / reporting catalogue**

All codes below are warnings (or info), not errors — none of them
block the apply. The result modal groups them and caps each group
at five visible rows with a `+ N more (total: X)` overflow line.

- Items:
  - `items_already_present` (warn) — entry already resolves on
    the live side, addMissing skipped it.
  - `unsupported_category` (warn) — category outside
    `editor.SupportedCategories`; entry skipped.
  - `items_layout_ignored` (info) — phase boundary marker;
    superseded by the layout writer (Phase 8E.1+). Still emitted
    when hand-authored YAML carries the legacy combo.
  - `items_template_override_ignored` (info) — template-baked
    weapon override overruled by the runtime override.
- Layout:
  - `layout_entry_missing` (warn) — entryRef did not resolve;
    entry skipped.
  - `layout_entry_ambiguous` (warn) — multiple live items
    matched; first match wins.
  - `layout_sparse_normalized` (info) — sparse / non-zero-based
    template positions normalised to compact `0..N-1`.
  - `layout_extra_items_preserved` (info) — count of live items
    not referenced by the template, appended after the prefix.
  - `layout_mode_unsupported` (warn) — mode outside the
    `reorderOnly` / `ignore` allowlist; section skipped.
- Weapon level override:
  - `weapon_level_clamped` (warn) — over-cap request clamped
    down via `editor.ClampUpgrade`.
  - `weapon_unupgradeable` (warn) — weapon with
    `MaxUpgrade == 0` requested an override.
- Session / mode / capacity errors:
  - `inventory_session_required` (error) — apply needs an
    active session it does not have.
  - `inventory_session_invalid` (error) — supplied session ID
    does not match any active session.
  - `inventory_capacity_exceeded` (error) — addMissing would
    cross the per-container cap.
- Result modal grouping (`ApplyItemsResultModal`) — same testid
  per group on every apply surface; all groups expose
  `data-warning-severity="info"` because the codes above are
  informational, not blocking. Layout counters appear in the
  `items-apply-result-layout-counters` section when any layout
  metric is non-zero.

**F. Non-goals deferred beyond Phase 8E.2**

- `applyOptions.{inventory,storage}Layout.mode = "append"` /
  `"replace"` — schema-allowed, writer-rejected, UI never sends.
- `applyOptions.items.mode = "updateExisting"` /
  `"merge"` / `"replace"` — schema-allowed, writer-rejected, UI
  never sends.
- Destructive delete / remove of live items via the template
  apply.
...
- ✅ **Shipped 2026-06-02 (Phase 7d.0)** — Core single-slot writer `PatchEquippedSpell(slot, slotIndex, spellID) error` for the 14-slot `EquippedSpells` region. Strict pre-validation (nil slot, slot-index range, uninitialised offset, out-of-bounds), idempotent no-op when target bytes already match, no hash mutation (the Phase 7d.3 batch writer owns hash[10]). Constants `EquippedSpellSlotCount = 14`, `EquippedSpellSlotSize = 8`, `EquippedSpellEmptySentinel = 0xFFFFFFFF`, `EquippedSpellOccupiedFollower = 0xFFFFFFFF`.
- ✅ **Shipped 2026-06-02 (Phase 7d.1)** — Schema additions in `backend/templates/schema.go`: `TemplateSections.Spells *SpellsSection` with 14 named pointer fields `Spell1..Spell14`, `TemplateSelection.Spells *SectionSelection`, `SpellSlotRef { BaseItemID uint32; Name string }`, constants `SpellSlotCount = 14` / `SpellItemIDPrefix = 0x40000000` / `SpellItemIDPrefixMask = 0xF0000000`, canonical iteration order `SpellSlotOrder []string` (`"spell1"..."spell14"`), `spellsSelectionFields` allowlist, validators `validateSpellsSelection` / `validateSpellsSection` / `validateSpellSlotRef` (reject any non-`0x4XXXXXXX` prefix at ingest), private getter `spellSlotRef`. Sorceries AND incantations share the `0x40000000` prefix in the SaveForge DB (`backend/db/data/sorceries.go`, `incantations.go`); the earlier briefing that suggested `0x60XXXXXX` for incantations was wrong and is intentionally **not** documented anywhere. `BaseItemID == 0` is the explicit-clear sentinel; the save-level `0xFFFFFFFF` never appears in the public schema. Nil pointer = leave the live slot unchanged.
- ✅ **Shipped 2026-06-02 (Phase 7d.2)** — Export builder + import preview/summary for spells. `ExportV2Options.EquippedSpellsRaw []uint32` (14 raw IDs from `slot.Data`; strict-reject when length ≠ `SpellSlotCount` AND `selection.spells` is selected); `buildSpellsSection(rawIDs, sel)` maps raw `0xFFFFFFFF` → `&SpellSlotRef{BaseItemID: 0}` and other raw → `&SpellSlotRef{BaseItemID: SpellItemIDPrefix | rawID}` with `Name` left empty (no DB lookup inside `backend/templates`). `ImportPreviewSummary.SpellSlotsPresent []string` populated via `spellSlotsPresent(tpl.Sections.Spells)`; `selectedSectionsForTemplate` adds `"spells"` when `tpl.Selection.Spells.HasAny()`. Apply still blocked until Phase 7d.3.
- ✅ **Shipped 2026-06-02 (Phase 7d.3)** — Backend apply path. `WriteSpells(writes []SpellWrite) error` (batch — pre-validates every write before any byte mutation, rejects duplicate slot indices, dispatches per write through `PatchEquippedSpell`, recomputes **only** hash[10] inline via `equipmentHash(readSpellIDs(slot.Data, slot.EquippedSpellsOffset))`; the global `RecalculateSlotHash` is NOT wired into production; `WriteEquipment` continues to own hash[7] / hash[8]). DB helper `db.ItemIDToMagicParamID(itemID uint32) uint32 { return itemID & 0x0FFFFFFF }` — **28-bit mask** mirroring `ItemIDToHandlePrefix`; a 16-bit mask would truncate high-payload spell IDs and is regression-guarded. `app_templates_v2_spells.go::resolveSpellWrites(slot, sel, sec)` — `!sel.Selected(key) || ref == nil` → no write; `BaseItemID == 0` → `EquippedSpellEmptySentinel`; `BaseItemID != 0` → defensive prefix re-check + DB membership check via `db.GetItemData(id).Category` (must be `"sorceries"` or `"incantations"`) + `db.ItemIDToMagicParamID` → raw spell ID; unknown valid-prefix → `IssueCodeUnknownItem` warning + skip. `ApplyTemplateV2Result.SpellSlotsApplied int`. Resolver dispatch placed **after** the equipment resolver and **before** the inventory resolver; `WriteSpells` call placed **after** `vm.MapViewModelToSlot` AND **after** `slot.WriteEquipment` — critical order because the VM flush rewrites the EquippedSpells region. Spells coexist freely with all other sections; no combo restrictions. Bindings regen (`frontend/wailsjs/go/models.ts` gains `spellSlotsApplied` + `spellSlotsPresent`; runtime files mode-bit flip only).
- ✅ **Shipped 2026-06-02 (Phase 7d.4)** — Frontend UI for spells. `V2_APPLY_SUPPORTED_SECTIONS` in `ImportTemplatePreviewModal.tsx` extended with `'spells'`; new `<div data-testid="import-preview-spell-slots">` row mirroring the equipment row; disabled-reason tooltip widened. `v2HasApplyableSections` in `TemplateLibraryModal.tsx` ORs in `selectedSections.includes('spells')`; library sections row renders `"spells"` verbatim through the existing `selectedSections.join(', ')` path. `TemplatesShellModal.tsx` unchanged — spells do not need a session. **`CreateTemplateV2Modal.tsx` intentionally untouched** — see Phase 7d.4b. Manual validation 2026-06-02 user-confirmed `manual OK`: Test A (full 14-slot loadout, 6 occupied + 8 explicit clear, `selection.spells: true`) and Test B (partial leave-unchanged, per-field selection of spell1/spell2/spell3 only, slots 4–14 retain pre-apply state) both passed end-to-end through Import preview → Apply → Save.
- ✅ **Shipped 2026-07-28 (Phase 8H — items summary fix)** — the import preview `Summary` counters (`inventoryItems`, `storageItems`, `weapons`, `armor`, `talismans`, `stackables`, `aowAssignments`) now derive from `sections.items` when a template ships v2 items instead of legacy `inventory.workspace`. Single shared `summarizeItemsSection` helper in `import.go`: container membership comes from `TemplateItemEntryV2.Location` — the SSOT — never from `inventoryLayout` / `storageLayout` (those describe ordering only); `location=both` counts the entry in both containers; category (`melee_armaments`/`ranged_and_catalysts`/`shields` → weapons, `head`/`chest`/`arms`/`legs` → armor, `talismans` → talismans, else stackables) and `ashOfWarItemID` are counted once per entry. Legacy `inventory.workspace` keeps priority and its historical counts; the two formats are never summed. `library.go::countInventoryItems` / `countStorageItems` reuse the same helper as a fallback, so `SaveTemplate` / `RebuildIndex` produce correct `LibraryTemplateEntry` counts for items-only templates (no `LibraryIndexVersion` bump — the DTO shape is unchanged). Frontend: `ImportTemplatePreviewModal.tsx` drops the stale "Apply / import flow lands in a later phase" copy for the clean-report note (now just "Template validated cleanly."), refreshes the read-only header comment, and gates the Apply-with-overrides weapon hint on `summary.weapons > 0` (armor/talisman/stackable-only item templates no longer show it) with copy scoped to "newly added weapons". No schema, apply-behaviour, or Wails-binding change. `sections.equipment` / `sections.spells` remain loadout references and are intentionally excluded from these item counters (their `EquipmentSlotsPresent` / `SpellSlotsPresent` surfaces are unchanged).
- ✅ **Shipped 2026-07-29 (Phase 7d.4b)** — Create-from-character export of `sections.equipment` and `sections.spells` from a loaded save (`CreateTemplateV2Modal` checkboxes + `BuildTemplateV2ExportOptions` wiring). Both sections come from one deep slot snapshot; empty supported slots export as explicit clears; equipment covers up to 18 supported slots (6 weapon / 4 ammo / 4 armor / active talisman1..4, locked talisman slots and talisman5 excluded), while spells exports the 10 player-visible slots normally or 12 while Moon of Nokstella is equipped (the two remaining physical save records are not exposed); equipment is disjoint from items/layout in a single export but combines freely with profile/stats/spells; Preview → Save to Library preserves the exact canonical JSON; equipment/spells apply needs no Sort Order session. See §17a.2 "Phase 7d.4b" for the full contract.
...
Phase 8+ work remains design-only in this document. Each phase requires a separate user approval before implementation per the workflow in `~/.claude/CLAUDE.md`.
