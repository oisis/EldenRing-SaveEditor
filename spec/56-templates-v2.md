...
- **Current contract**:
  ...
  - Equipment **cannot** be combined in a create-from-character export with `items` / `inventoryLayout` / `storageLayout` (rejected in `BuildV2Template` and at the UI — equipment applies against items already present in the target's inventory). Equipment **may** be combined with `profile` / `stats` / `spells`.
