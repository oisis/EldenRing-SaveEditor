...
§17a.2j). The follow-up Phase 7d.4b (create-from-character export +
`CreateTemplateV2Modal` checkbox) remains planned (see below).
...
#### Phase 7d.4b — create-from-character export bridge for spells (planned)

- **Goal**: extend the backend create-from-character flow so the user can build a spells template directly from a loaded save (today producers must hand-author the YAML).
- **Planned scope**:
  - `BuildTemplateV2ExportOptions` (in `app_templates_v2.go`) gains `EquippedSpellsRaw []uint32` (14 raw IDs).
  - `buildAndValidateTemplateV2FromCharacter(charIdx, selectionJSON, opts)` reads `slot.Data` at the EquippedSpells region (via `core.readSpellIDs` or equivalent) and passes the result into `templates.BuildV2Template` whenever `selection.spells.HasAny()`. The existing strict-length reject in `buildSpellsSection` (Phase 7d.2) makes this safe by construction.
  - `frontend/src/components/templates/CreateTemplateV2Modal.tsx` adds a `Spells` checkbox following the existing per-section pattern (profile / stats — equipment is in the same gated state and will likely land alongside spells in this same bridge phase).
- **Why deferred**: the current create modal would let the user select Spells and then silently discard the data (the backend export path does not yet carry `EquippedSpellsRaw`). Adding the checkbox without the backend bridge is "dead UI" — exactly the situation `~/.claude/CLAUDE.md` forbids. The same gap applies to equipment, which is in the same deferred state for the same reason.
- **Risks**: low — the writer / preview / apply / frontend gating are already shipped (Phase 7d.0 → 7d.4); this bridge is read-side only.
- **Out of scope**: any apply-path change.
...
- Create-from-character export of `sections.spells` from a loaded save (`BuildTemplateV2ExportOptions.EquippedSpellsRaw`-side wiring and the `CreateTemplateV2Modal` Spells checkbox) — gated by Phase 7d.4b. Today producers must hand-author the spells YAML; the apply / preview / library / bindings surface is fully ready.
