...
// Slot-data layout of the confirmed GestureGameData section, shared by PC and
// PS4. The section has no fixed position inside a slot: it sits directly behind
...
// declares. This getter owns its own anchor, its own layout constants and its own
// bounds checks; it borrows no position, helper or parsing function from another
// getter.
const (
	...
	// read, decoded or validated by GetGestures.
...
func (engine *Engine) GetGestures(saveSessionID string, characterID int) (CharacterGestures, error) {
	...
	if err != nil {
		return CharacterGestures{}, fmt.Errorf(
			"cannot search the gestures of character %d: %w", characterID, err)
	}
	if anchor < 0 {
		return CharacterGestures{}, fmt.Errorf("character %d carries no gesture anchor", characterID)
	}
...
// gestureSlotBounds selects the platform entry point of this getter. PC and PS4
