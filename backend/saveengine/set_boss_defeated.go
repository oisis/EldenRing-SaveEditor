...
// SetBossDefeatedResult reports one committed defeat state change.
//
...
// flags of every curated boss lie in. The catalog enforces the same bound on its
// documents; this method restates it so a flag of another block can never be
// flipped through the boss entry point.
const bossFlagBlock = 9

// SetBossDefeated sets or clears the event flag bit associated with a boss
// resource in one character slot of an existing session.
//
...
//
// eventFlagID is the confirmed synchronized defeat event flag of the boss. Only
// block 9 is accepted; any other block is rejected.
//
...
func (engine *Engine) SetBossDefeated(
	...
	if !isCanonicalRevision(expectedRevision) {
		return SetBossDefeatedResult{}, apperror.InvalidRevision(expectedRevision)
	}
	if block := eventFlagID / eventFlagsPerBlock; block != bossFlagBlock {
		return SetBossDefeatedResult{}, fmt.Errorf(
			"event flag %d lies in block %d, which is not the confirmed boss block %d",
			eventFlagID, block, bossFlagBlock)
	}
