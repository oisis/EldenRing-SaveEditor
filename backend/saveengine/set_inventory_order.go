...
const (
	inventoryOrderReservedIndexMax uint32 = 432
	inventoryOrderUnsafeIndex      uint32 = 10000
)
...
func (engine *Engine) SetInventoryOrder(
	...
	saveRevision, err := engine.commitRevision(saveSessionID, func(loaded *loadedSave) error {
		...
		indices, err := planInventoryOrderIndices(storedNext, len(ordered), retainedBuckets)
		if err != nil {
...
func planInventoryOrderIndices(
	storedNext uint32,
	count int,
	retainedBuckets map[uint32]struct{},
) ([]uint32, error) {
	base := uint64(storedNext)
	if base <= uint64(inventoryOrderReservedIndexMax) {
		base = uint64(inventoryOrderReservedIndexMax) + 2
	}
	if base%2 != 0 {
		base++
	}
	...
	if last >= uint64(inventoryOrderUnsafeIndex) {
		return nil, fmt.Errorf(
			"Inventory order would assign acquisition index %d, want at most %d",
			last, inventoryOrderUnsafeIndex-1)
	}
