...
const (
	storageTestPCSlotDataBase  = 0x300 + 0x10 // first PC slot data, behind its MD5 prefix
	storageTestPCSlotStride    = 0x280010
	storageTestPS4SlotDataBase = 0x70 // first PS4 slot data, no MD5 prefix
	storageTestPS4SlotStride   = 0x280000
	storageTestSlotDataSize    = 0x280000
	...
	storageTestProjectileCountAt = 0xD0 + 0x58 + 0x1C + 0x58 + 0x58 + 0x9011 + 0x74 + 0x8C + 0x18
	storageTestProjectileStride  = 8
	...
	storageTestBlocksBefore = 0x9C + 0x0C + 0x12F
	...
	// trailing counters.
	storageTestRecordSize  = 12
	storageTestCommonAt    = 4
	storageTestCommonSize  = 0x780 * storageTestRecordSize
	storageTestKeyAt       = storageTestCommonAt + storageTestCommonSize + 4
	storageTestKeySize     = 0x80 * storageTestRecordSize
	storageTestSectionSize = storageTestKeyAt + storageTestKeySize + 8
)

// storageTestAnchor is the 65-byte anchor the storage chain is measured from,
...
// storageTestRow is one raw record written into a synthetic section, with the
// stored quantity exactly as the game keeps it, including the high bit.
type storageTestRow struct {
	index       int
	handle      uint32
	rawQuantity uint32
	acquisition uint32
...
// storageTestFixture describes the synthetic slot content one test save is built
// from. A residual slot is expressed as a zero flag with everything still
// written into the file, and projectiles shifts the section the way an acquired
...
type storageTestFixture struct {
	...
	anchorAt    int64
	projectiles uint32
	common      []storageTestRow
	key         []storageTestRow
	noAnchor    bool
}

// writeStorageFixture builds a synthetic save and returns its path inside
// t.TempDir(). Only the activity flag, the anchor, the projectile count and the
// requested records are written; the rest of the container stays zeroed, which
// is the native empty sentinel of a record. A value that would reach past the
// end of the slot data is left out, which is how the truncated case is
// expressed.
func writeStorageFixture(t *testing.T, content storageTestFixture) string {
	t.Helper()
	...
	case PlatformPC:
		...
		slotBase = storageTestPCSlotDataBase + int64(content.slot)*storageTestPCSlotStride
	case PlatformPS4:
		data = make([]byte, ps4FixtureSize)
		copy(data, ps4Header())
		userData10Base = ps4UserData10DataOffset
		slotBase = storageTestPS4SlotDataBase + int64(content.slot)*storageTestPS4SlotStride
	default:
	...
	copy(data[slotBase+content.anchorAt:], storageTestAnchor)

	countAt := content.anchorAt + storageTestProjectileCountAt
	if countAt+4 <= storageTestSlotDataSize {
		binary.LittleEndian.PutUint32(data[slotBase+countAt:], content.projectiles)
	}
	sectionAt := countAt + 4 +
		int64(content.projectiles)*storageTestProjectileStride + storageTestBlocksBefore

	putRow := func(sectionOffset int64, row storageTestRow) {
		at := sectionAt + sectionOffset + int64(row.index)*storageTestRecordSize
		if at+storageTestRecordSize > storageTestSlotDataSize {
			return
		}
	...
	for _, row := range content.common {
		putRow(storageTestCommonAt, row)
	}
	for _, row := range content.key {
		putRow(storageTestKeyAt, row)
	}
	return write()
}

// storageTestCommonRows and storageTestKeyRows deliberately mix occupied rows
// with both native absent sentinels and leave gaps between them, so a reader
// that renumbers rows, drops the wrong sentinel or reorders the sections cannot
// pass. The last row of each section is its highest physical index, so a reader
// using the Inventory section sizes instead of the Storage ones cannot pass
// either.
func storageTestCommonRows() []storageTestRow {
	return []storageTestRow{
		...
		{index: 0x77F, handle: 0x80000002, rawQuantity: 0xFFFFFFFF, acquisition: 0xFFFFFFFF},
	}
}

func storageTestKeyRows() []storageTestRow {
	return []storageTestRow{
		...
		{index: 0x7F, handle: 0x00000003, rawQuantity: 0x00000002, acquisition: 40},
	}
}

// storageTestWantCommon and storageTestWantKey are the records the two row sets
// must produce: the sentinels are gone, the physical index of every remaining
// row is preserved, and the high bit of the quantity is masked off while the
// handle and the acquisition index stay untouched.
func storageTestWantCommon() []StorageRecord {
	return []StorageRecord{
		...
		{ContainerSection: "common", PhysicalIndex: 0x77F, GaItemHandle: 0x80000002,
			Quantity: 0x7FFFFFFF, AcquisitionIndex: 0xFFFFFFFF},
	}
}

func storageTestWantKey() []StorageRecord {
	return []StorageRecord{
		{ContainerSection: "key", PhysicalIndex: 2, GaItemHandle: 0xC0000001, Quantity: 1, AcquisitionIndex: 12},
		{ContainerSection: "key", PhysicalIndex: 0x7F, GaItemHandle: 0x00000003, Quantity: 2, AcquisitionIndex: 40},
	}
}
...
func storageTestActiveFixture(
	...
	return storageTestFixture{
		platform: platform, slot: slot, flag: 1, anchorAt: anchorAt, projectiles: projectiles,
		common: storageTestCommonRows(), key: storageTestKeyRows(),
	}
}

func TestGetStorageReadsTheActiveSlotOfBothPlatforms(t *testing.T) {
	...
	// front of the section, cannot pass both cases.
	...
	for _, content := range cases {
		t.Run(string(content.platform), func(t *testing.T) {
			...
			result, err := engine.GetStorage(loaded.SaveSessionID, content.slot, "", 0, 0)
			if err != nil {
...
func TestGetStorageFiltersBySection(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"common": {StorageSectionCommon, storageTestWantCommon()},
		"key":    {StorageSectionKey, storageTestWantKey()},
		"both":   {"", append(storageTestWantCommon(), storageTestWantKey()...)},
	}
	for name, testCase := range cases {
		t.Run(name, func(t *testing.T) {
			result, err := engine.GetStorage(loaded.SaveSessionID, 3, testCase.section, 0, 0)
			if err != nil {
				t.Fatalf("GetStorage: %v", err)
			}
...
func TestGetStoragePagesWithoutLosingTheNativeOrder(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"beyond the total":    {9, 2, 9, 2, []StorageRecord{}},
		"one page for all":    {1, 0, 1, 50, all},
	}
	for name, testCase := range cases {
		t.Run(name, func(t *testing.T) {
			result, err := engine.GetStorage(loaded.SaveSessionID, 1, "", testCase.page, testCase.pageSize)
			if err != nil {
				t.Fatalf("GetStorage: %v", err)
			}
...
func TestGetStorageReportsAResidualSlotAsInactive(t *testing.T) {
	content := storageTestActiveFixture(PlatformPC, 4, 0x0800, 0)
	content.flag = 0
...
func TestGetStorageRejectsInvalidRequests(t *testing.T) {
	...
	present := loadSlot(storageTestActiveFixture(PlatformPC, 2, 0x0640, 0))
	missingAnchor := loadSlot(storageTestFixture{
		platform: PlatformPC, slot: 2, flag: 1, noAnchor: true,
	})
	// The anchor sits so close to the end of the slot that the last trailing
	// counter of the section no longer fits inside the slot data.
	truncated := loadSlot(storageTestFixture{
		platform: PlatformPC, slot: 2, flag: 1,
		anchorAt: storageTestSlotDataSize -
			(storageTestProjectileCountAt + 4 + storageTestBlocksBefore + storageTestSectionSize - 1),
	})
	// A declared projectile count far beyond anything a native save carries is
	// corrupt, not a position to follow.
	corruptCount := loadSlot(storageTestFixture{
		platform: PlatformPC, slot: 2, flag: 1, anchorAt: 0x0640, projectiles: 200001,
	})
	// An accepted count that still pushes the section past the end of the slot
	// is rejected before anything is read.
	shiftedOutOfSlot := loadSlot(storageTestFixture{
		platform: PlatformPC, slot: 2, flag: 1, anchorAt: 0x200000, projectiles: 100000,
	})

	closed := loadSlot(storageTestActiveFixture(PlatformPC, 2, 0x0640, 0))
	...
	cases := map[string]struct {
		...
		"inventory alias": {present, 2, "storage", 0, 0,
		...
		"missing anchor":    {missingAnchor, 2, "", 0, 0, "character 2 carries no storage anchor"},
		"truncated section": {truncated, 2, "", 0, 0,
			"storage of character 2 does not fit into its slot"},
		"corrupt projectile count": {corruptCount, 2, "", 0, 0,
			"character 2 declares 200001 projectile records, want at most 200000"},
		"section pushed out of the slot": {shiftedOutOfSlot, 2, "", 0, 0,
			"storage of character 2 does not fit into its slot"},
	}

	for name, testCase := range cases {
		t.Run(name, func(t *testing.T) {
			result, err := engine.GetStorage(
			...
			if !reflect.DeepEqual(result, CharacterStorage{}) {
