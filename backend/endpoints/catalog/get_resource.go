...
GameCatalog variables read: the resource stored under the given (Resource.Kind, Resource.Key) pair including the full document of its kind: for kind item the schema.ItemDocument (presentation, capabilities, safety, storage, acquisition, modifiers, links, variants, aliases, unlocks, related technical records, source records and family data), for kind colosseum the schema.ColosseumDocument (name and unlock event flag ID, each with its own provenance), for kind region the schema.RegionDocument (region ID, name and area, each with its own provenance).
