...
GameCatalog variables read: the resource stored under the given (Resource.Kind, Resource.Key) pair including the full document of its kind: for kind item the schema.ItemDocument (presentation, capabilities, safety, storage, acquisition, modifiers, links, variants, aliases, unlocks, related technical records, source records and family data), for kind colosseum the schema.ColosseumDocument (name and unlock event flag ID, each with its own provenance), for kind region the schema.RegionDocument (region ID, name and area, each with its own provenance), for kind summoning_pool the schema.SummoningPoolDocument (name, region label and activation event flag ID, each with its own provenance), for kind grace the schema.GraceDocument (name, region label, visit event flag ID, boss-arena fact, dungeon type and door event flag ID, each with its own provenance), for kind boss the schema.BossDocument (name, region label, encounter type, remembrance fact and defeat event flag ID, each with its own provenance), for kind map_region the schema.MapRegionDocument (name, area label and visibility event flag ID, each with its own provenance), for kind tutorial the schema.TutorialDocument (TutorialParam row ID and official title, each with its own provenance), for kind quest the schema.QuestDocument (the quest name with its own provenance, the supported steps whose descriptions and locations each carry their own provenance, and their canonical event flag plans, which declare an ID and a target value only and carry no per-flag provenance), for kind class the schema.ClassDocument (starting class ID, name, base level and the eight base attributes, each with its own provenance).
