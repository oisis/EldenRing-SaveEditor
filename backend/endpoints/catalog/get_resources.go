...
GameCatalog variables read: Resource.Kind, Resource.Key, Item.Family, Item.Presentation.Name, Item.Capabilities (Known and Enabled only), Colosseum.Name, Region.Name, SummoningPool.Name, Grace.Name and Boss.Name. The full resource document is never projected; it stays the responsibility of GetResource.
...
// grace and boss; family and capability describe items only, so a non-empty one
