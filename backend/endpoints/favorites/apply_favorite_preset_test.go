...
func writeEndpointApplyFavoritesFixture(t *testing.T, charSlot int, favSlot int) string {
	t.Helper()
	...
	// Active favorite preset slot
	slotAt := base + 0x154 + int64(favSlot)*0x130
	binary.LittleEndian.PutUint16(data[slotAt:], 0xFACE)
	binary.LittleEndian.PutUint32(data[slotAt+0x04:], 0x11D0)
	data[slotAt+0x08] = 1
	data[slotAt+0x09] = 0 // male
	...
	path := filepath.Join(t.TempDir(), "endpoint_apply_favorites.sl2")
...
func TestApplyFavoritePresetEndpointSuccess(t *testing.T) {
	path := writeEndpointApplyFavoritesFixture(t, 0, 3)
	...
	if result.CharacterID != 0 {
		t.Errorf("CharacterID = %d, want 0", result.CharacterID)
	}
...
func TestApplyFavoritePresetEndpointRejectsMissingEngine(t *testing.T) {
	if _, err := ApplyFavoritePreset(nil, "session", 0, 0, "0"); err == nil {
		t.Fatal("expected error for nil engine, got nil")
	}
}

func TestApplyFavoritePresetEndpointDelegatesValidation(t *testing.T) {
	path := writeEndpointApplyFavoritesFixture(t, 0, 3)
	engine := saveengine.New()
	session, err := engine.LoadSave(path, "pc")
	if err != nil {
	...
	if _, err := ApplyFavoritePreset(engine, session.SaveSessionID, 0, 15, "0"); err == nil {
