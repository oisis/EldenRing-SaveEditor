...
Purpose: Returns InventoryHeld records from one character slot resolved to ItemDocument identities.
How it works: The runtime handler reads the records and their save-side game IDs through SaveEngine, then resolves each ID through the already loaded GameCatalog. The endpoint opens no file and parses no save data of its own.
...
Save variables read: the UserData10 activity flag of the requested slot and, for an active slot, the physical InventoryHeld records and GaItem table; the getter is non-mutating, keeps gaItemHandle and acquisitionIndex raw, masks only the documented high bit of quantity and applies paging.
...
var GetInventoryDefinition = contract.MustDefine(contract.Definition{
	...
	Description:                "Returns InventoryHeld records from one character slot resolved to ItemDocument identities.",
})
...
// GetInventoryResult is one resolved page of InventoryHeld records.
...
// GetInventory returns one page of InventoryHeld records stored in one
// character slot of an existing save session. Every listed record is resolved
// to one ItemDocument by its GaItem-backed game ID. The result retains the
// physical fields from the raw reader, but no name, family filter, equipped
// state, capacity or Storage record is added here.
func GetInventory(
	...
	if gameCatalog == nil {
		return GetInventoryResult{}, errors.New("game catalog is not available")
	}

	stored, err := engine.GetInventory(saveSessionID, characterID, containerSection, page, pageSize)
	if err != nil {
		return GetInventoryResult{}, err
	}
	result := GetInventoryResult{
		...
		Records:       make([]InventoryRecord, 0, len(stored.Records)),
		Total:         stored.Total,
		Page:          stored.Page,
	...
	for index, record := range stored.Records {
		...
		if !exists || resource.Kind != schema.ResourceKindItem || resource.Item == nil || resource.Key == "" {
			return GetInventoryResult{}, fmt.Errorf("inventory record %d: game ID 0x%08X is not a known item",
				index, gameIDs[index])
		}
		result.Records = append(result.Records, InventoryRecord{
