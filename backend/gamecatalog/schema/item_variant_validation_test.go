...
func validExtendedWeaponData(p schema.Provenance, data schema.WeaponData) *schema.WeaponData {
	...
	data.StatusPoison = knownFact(p, int32(0))
	data.StatusBleed = knownFact(p, int32(0))
	data.StatusFrost = knownFact(p, int32(0))
	data.StatusSleep = knownFact(p, int32(0))
	data.StatusMadness = knownFact(p, int32(0))
	data.StatusScarletRot = knownFact(p, int32(0))
